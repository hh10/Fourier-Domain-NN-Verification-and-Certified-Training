...
def get_params(model):
    ...
    for p in model.named_parameters():
        if any([k in p[0] for k in ['L_W1_r', 'L_W1_i', 'L_W2_r', 'L_Dinv', 'channel_broadcaster']]):
            print(f'Skipping parameter {p[0]}')
        else:
            if 'weight' in p[0]:
                weights.append(p)
                print(f'Adding weight for {p[0]}')
            elif 'bias' in p[0]:
                biases.append(p)
                print(f'Adding bias for {p[0]}')
            else:
                print(f'Skipping parameter {p[0]}')
...
def ibp_init(model_ori, model):
    from auto_LiRPA.bound_ops import BoundConv, BoundLinear, BoundAdd
...
def orthogonal_init(model_ori, model):
    params = []
    bns = []
...
def manual_init(init_method, model_ori, model):
    if init_method == 'ibp':
        ibp_init(model_ori, model)
    elif init_method == 'orthogonal': 
        orthogonal_init(model_ori, model)
    elif init_method == 'kaiming':
        print('Initialization: Kaiming normal')
